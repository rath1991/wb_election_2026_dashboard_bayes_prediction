...
if page == "Headline Forecast":
    ...
    with col_gauge:
        fig = go.Figure(go.Indicator(
            ...
            gauge={
                ...
                "borderwidth": 0,
                "steps": [
                    {"range": [50,  120], "color": "rgba(239,68,68,0.10)"},
                    {"range": [120, 148], "color": "rgba(245,158,11,0.10)"},
                    {"range": [148, 250], "color": "rgba(34,197,94,0.08)"},
                ],
                "threshold": {"line": {"color": "#f8fafc", "width": 2},
                              "thickness": 0.8, "value": 148},
            },
        ))
        fig.add_annotation(
            text=f"Likely: {p25}–{p75}  ·  90% CI: {forecast['tmc_p5']}–{forecast['tmc_p95']}",
    ...
    with col_donut:
        ...
        fig2 = go.Figure(go.Pie(
            labels=labels, values=probs, hole=0.62,
            marker={"colors": colors_d, "line": {"color": "#0e1117", "width": 2}},
            textinfo="label+percent", textfont={"size": 11}, rotation=90,
        ))
        # Embed scenario probabilities inside the donut hole
        fig2.add_annotation(
            text="<b>Scenarios</b>", x=0.5, y=0.62,
        ...
        for i, (txt, col) in enumerate([
            ...
            fig2.add_annotation(
                ...
                xref="paper", yref="paper", showarrow=False,
                font={"size": 9.5, "color": col}, align="center",
            )
        fig2.update_layout(
    ...
    # ── Scenario cards with probabilities ───────────────────────────────────
    ...
    for s in SCENARIOS_VIZ:
        with s["col"]:
            st.markdown(
                f"<div style='border:1px solid {s['color']};border-radius:8px;padding:12px;"
                f"background:rgba(14,17,23,0.6);text-align:center'>"
                f"<div style='font-size:1.1em;font-weight:bold;color:{s['color']}'>{s['name']}</div>"
                f"<div style='font-size:2em;font-weight:900;color:{s['color']};margin:4px 0'>{s['prob']}</div>"
                f"<div style='font-size:1.3em;color:#f8fafc'>{s['seats']} seats</div>"
                f"<div style='font-size:0.85em;color:#94a3b8;margin-top:6px;text-align:left'>{s['drivers']}</div>"
                f"</div>",
                unsafe_allow_html=True,
            )
    ...
    st.markdown(
        ...
        "padding:10px 14px;border-radius:4px;margin-top:12px;font-size:0.9em'>"
        "🚨 <b>Bhabanipur watch:</b> 51,004 deletions vs 2024 LS margin of 8,297 = <b>6.15× pressure.</b> "
        "BJP led 149 of 269 booths in 2024. If this seat flips it signals BJP Surge scenario is live. "
        "40% of post-adjudication deletions were Muslim names despite 20% Muslim population "
        "<i>(Sabar Institute / Telegraph India)</i>."
        "</div>",
        unsafe_allow_html=True,
    )
